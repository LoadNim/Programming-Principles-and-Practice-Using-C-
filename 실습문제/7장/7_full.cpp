...
const char Exit   = 'e';    // t.kind==quit means that t is a quit Token
const char print  = ';';    // t.kind==print means that t is a print Token
const char name   = 'a';    // name token
const char let    = '#';    // declaration token
const char con    = 'C';    // const declaration token
const char SQRT   = 'S';    // sqrt token
const char POW    = 'P';    // pow token
...
Token Token_stream::get() // read characters from cin and compose a Token
{
    ...
    default:
        if (isalpha(ch)) {	// start with a letter
            ...
            if (s == "sqrt") return Token(SQRT); // keyword "sqrt"
            if (s == "pow") return Token(POW); // keyword "pow"
            return Token(name,s);
        }
        error("Bad token");
    }
...
double primary()
{
    ...
    case POW:{
        ...
        double left = primary();
        cin>>c;
        int right = static_cast<int>(primary());
        cin>>c;
    ...
    case SQRT:{
        double d = expression();
        if(d < 0) error("음수의 제곱근은 정의되지 않습니다.");
        return sqrt(d);
    }
    case '-':
        return - primary();
